...
// ── Integration setup ─────────────────────────────────────────────────
// One surface, two audiences. The employer configures their own delivery
// tracking here, and Kassab staff open the very same panel for a company
// they support — so "where do I set this up?" has a single answer instead
// of one screen for setup and another for monitoring.
export function IntegrationSetupPanel({
  companyId, drivers, ordersPath, trackingPath, readOnly = false,
}: {
  companyId: string
  drivers: Driver[]
  ordersPath: string
  ...
  const integration = useMemo(
    () => integrations.find((i) => i.companyId === companyId && i.status !== 'disconnected'),
  ...
  const linked = externalIdentities.filter((x) => x.companyId === companyId)
    ...
    return (
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="border-brand-200">
          ...
          <Button className="mt-4 w-full" disabled={readOnly} onClick={() => setConnectOpen('system')}>
            {t('int.connectSystem')}
          </Button>
        </Card>
        ...
        <Card className="border-emerald-200">
          ...
          <Button variant="success" className="mt-4 w-full" disabled={readOnly} onClick={() => setConnectOpen('kassab')}>
            {t('int.useKassab')}
          </Button>
        </Card>

        <div className="lg:col-span-2">
          <ApiAccessCard companyId={companyId} readOnly={readOnly} />
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          ...
          <Card title={t('int.whatKassabSees')}>
            ...
            <Link to={trackingPath} className="mt-4 block">
